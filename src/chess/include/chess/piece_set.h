...
namespace chess {
    struct PieceSet {
        ...
        }, piece_count(invalid ? 0 : 16) {}

        auto constexpr cleanup() {
        ...
        struct ReferencedPiece {
            ...
            constexpr ReferencedPiece(Piece& piece, PieceSet* parent) : piece(piece), need_cleanup(false), parent(parent) {}
        ...
        auto constexpr grab_piece(const Piece& piece) -> ReferencedPiece {
            for(u8 i=0; i<piece_count; ++i) {
                ...
                if(my_piece == piece) return {my_piece, this};
            }
            nespp::Error::fatal_error("Illegal piece grab", 18);
            return { pieces[0], nullptr};
        }
        ...
        auto constexpr grab_invalid(const PieceType piece_type) -> ReferencedPiece {
            for(u8 i=piece_count; i<16; ++i) {
                auto & my_piece = pieces[i];
                if(my_piece.piece_type() == piece_type) return {my_piece, this};
            }
